...
document.addEventListener('click', (e) => {
    chrome.storage.sync.get({'toggle' : 'on'}, (result)=> {
...
function convertCurrency(e) {
    ...
    'boxFontLink': ''
        ...
        if (result.decimalSeparator === 'period') {
            let selectedNum = Number(document.getSelection().toString().replace(/,/g, ''));
            if (!selectedNum) return;
            testKeyHeld(result, selectedNum);
        } else {
            let selectedNum = Number(document.getSelection().toString().replace(/\./g, '').replace(/,/g, '.'));
            if (!selectedNum) return; 
            testKeyHeld(result, selectedNum);
        }
    ...
    function getRates(result, selectedNum) {
        let rate = result.latestRates.rates[result.to] / result.latestRates.rates[result.from];
        let convertedNum = (selectedNum * rate).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
...
function displayPopup(result, convertedNum) {
    ...
    let selectedLocation = document.getSelection().getRangeAt(0).getBoundingClientRect();
    ...
    box.style.left = selectedLocation.left + (selectedLocation.width / 2) + window.pageXOffset + 'px';
    ...
    arrowBorder.style.top = selectedLocation.bottom + 1 + window.pageYOffset + 'px';
    arrowBorder.style.left = selectedLocation.left + (selectedLocation.width / 2) + window.pageXOffset + 'px';
    ...
    arrow.style.left = selectedLocation.left + (selectedLocation.width / 2) + window.pageXOffset + 'px';
    ...
    if (result.position === 'above') {
        box.style.top = selectedLocation.top - boxPos.height - 10 + window.pageYOffset + 'px';
        boxPos = box.getBoundingClientRect();
        arrowBorder.style.top = selectedLocation.top - 10 + window.pageYOffset + 'px';
        ...
        if (boxPos.left < 0) { 
            ...
            box.style.transform = 'none';
            box.style.left = (document.documentElement.clientWidth - boxPos.width) + window.pageXOffset + 'px';
            if (boxPos.top < 0) {
                box.style.top = selectedLocation.bottom + 10 + window.pageYOffset + 'px';
                arrowBorder.style.top = selectedLocation.bottom + 1 + window.pageYOffset + 'px';
                arrowBorder.style.borderTop = null;
